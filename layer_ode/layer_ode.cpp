    ...
    public:
        ...
        coordinate GradientEquationResidual(BezierImage obj, int cpt, std::vector<int> samples) override
        {
            ...
            for (int i = 0; i < samples[0]+1; i++)
            {
                ...
                result.coord[0] = result.coord[0] + 2.0*(0.1*v00.coord[0] + 2.0*v0.coord[0] + exp(v.coord[0]))*(0.1*obj.CPtFactorOfDoubleDiffBezierMap(cpt, ts, 0 ,0) + 2.0*obj.CPtFactorOfDiffBezierMap(cpt, ts, 0) + exp(v.coord[0])*obj.CPtFactorOfBezierMap(cpt, ts));

                if (i==0 || i== samples[0])
                {
                    result.coord[0] = result.coord[0] + 200000.0*(v.coord[0])*obj.CPtFactorOfBezierMap(cpt, ts);
                }
                ts[0] = ts[0] + timeGap;
            }            
...
{    
    ...
    obj.ReadFile("data/layer_param.txt");

    SystemCalculator calculator;

    BezierImage optimizedResult = calculator.ConjugateGradientOptimization(obj, samples, 0.2);
